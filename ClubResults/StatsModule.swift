...
private struct VoiceReviewSuggestion: Identifiable {
    let id = UUID()
    let statTypeId: UUID
    let playerId: UUID
...
struct LiveStatsView: View {
    ...
    @State private var reviewSuggestion: VoiceReviewSuggestion?
#if DEBUG
    @State private var voiceDebugText: String?
#endif
...
#endif
                if let suggestion = reviewSuggestion {
                    HStack(spacing: 6) {
                        Text("Did you mean \(statName(for: suggestion.statTypeId)) — \(playerLabel(for: suggestion.playerId))?")
                            .font(.caption)
                        Button("Confirm") {
                            addSuggestedVoiceEvent(suggestion)
                        }
                        .buttonStyle(.borderedProminent)
                        Button("Dismiss") {
                            reviewSuggestion = nil
                        }
                        .buttonStyle(.bordered)
                    }
                }
...
#if DEBUG
        print("VOICE_PARSE \(result.debugLog)")
        voiceDebugText = result.debugLog
#endif

        guard result.parseStatus == .success,
              let statTypeId = result.matchedStatTypeId,
              let playerId = result.matchedPlayerId else {
            if result.parseStatus == .lowConfidence,
               let suggestionStatId = result.bestGuessStatTypeId,
               let suggestionPlayerId = result.bestGuessPlayerId {
                reviewSuggestion = VoiceReviewSuggestion(
                    statTypeId: suggestionStatId,
                    playerId: suggestionPlayerId,
                    transcript: result.rawTranscript,
                    normalizedTranscript: result.normalizedTranscript,
                    confidence: result.confidence
                )
                lastMessage = "Low confidence. Review suggestion."
            } else {
                reviewSuggestion = nil
    ...
    private func addSuggestedVoiceEvent(_ suggestion: VoiceReviewSuggestion) {
        let event = StatsEventCreationService.makeVoiceEvent(
            sessionId: session.sessionId,
            playerId: suggestion.playerId,
            statTypeId: suggestion.statTypeId,
            quarter: selectedQuarter,
            transcript: suggestion.transcript,
            normalizedTranscript: suggestion.normalizedTranscript,
            confidence: suggestion.confidence
        )
        modelContext.insert(event)
        try? modelContext.save()
        reviewSuggestion = nil
        lastMessage = "Added: \(statName(for: suggestion.statTypeId)) — \(playerLabel(for: suggestion.playerId)) — \(selectedQuarter)"
        feedbackToken = UUID()
    }
    ...
    private func parseFailureMessage(_ result: VoiceParseResult) -> String {
        ...
        case .ambiguousPlayer:
            if let first = result.candidatePlayerNames.first {
                return "Multiple players match '\(first)'"
            }
